...
export default {
    ...
    updateDetails: async (req, res, next) => {
        try {
            const { dob, contact } = req.body;
    
            // Fetch user details
            const user = await getUser(req, res, next); // Updated here
            ...
            await DB.execute(
                'UPDATE users SET date_of_birth=?, contact=?, age=? WHERE id=?',
                [dob, contact, age, userId]
            );
